...
{The thesis describes differents problems leading to possible ineffectivity in usage of Java memory. It also investigates whether such problems actually exist in commonly used software and if and how can be the problems detected. It defines several types of these problems, e.g. duplicate objects and infeffective usage of the Java collections. After that it describes possible ways to analyze Java memory and based on that, the offline Java memory dump processing approach is chosen. As a result of that, an application is created for such purpose. It is capable of searching for and marking the defined memory issues. The application is later tested on a laboratory program and also commonly used software -- an instance of the Spring Boot framework.}

%% Klíčová slova
\klicovaslova{Java, halda, memory dump, plýtvání pamětí, analýza paměti, hprof, duplicitní objekt \\ }{Java, heap, memory dump, memory wasting, memory analysis, hprof, duplicate object \\ }

%% Poděkování
%\podekovanitext{Rád bych poděkoval vedoucímu bakalářské práce panu Ing.\ Richardu Lipkovi,\ Ph.D.\linebreak za~odborné vedení, vstřícný přístup, trpělivost a podnětné návrhy k~práci.}
